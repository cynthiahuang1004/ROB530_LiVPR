...
def eval_nordland(model, image_size=(322, 322), batch_size=64, device='cuda'):
    print("\n===== NordLand =====")
...
def eval_pitts250k(model, image_size=(322, 322), batch_size=64, device='cuda'):
    print("\n===== Pitts250k-test =====")
    transform = get_transform(image_size)
    ...
    loader = DataLoader(dataset, batch_size=batch_size,
                        num_workers=8, shuffle=False)
    feats = evaluate(model, loader, device)
    num_references = dataset.dbStruct.numDb
    r_list = feats[:num_references]
    q_list = feats[num_references:]
    ...
    pitts_dict, _ = utils.get_validation_recalls(
        ...
        gt=positives,
        print_results=True,
        dataset_name='Pitts250k-test',
        faiss_gpu=False
    )
    return pitts_dict

def eval_sped(model, image_size=(322, 322), batch_size=64, device='cuda'):
    print("\n===== SPED =====")
    transform = get_transform(image_size)
    ...
    r_list = evaluate(model, ref_loader, device)
    q_list = evaluate(model, query_loader, device)
...
def eval_msls(model, image_size=(322, 322), batch_size=64, device='cuda'):
    print("\n===== MSLS Val =====")
    transform = get_transform(image_size)
    ...
    for city in val_cities:
        csv_path = os.path.join(msls_path, 'train_val', city, 'database', 'subtask_index.csv')
        raw_path = os.path.join(msls_path, 'train_val', city, 'database', 'postprocessed.csv')
        idx_df = pd.read_csv(csv_path)
        raw_df = pd.read_csv(raw_path)
        val_keys = idx_df[idx_df['all'] == True]['key'].tolist()
        for key in val_keys:
            row = raw_df[raw_df['key'] == key].iloc[0]
            ref_utms.append([row['easting'], row['northing']])
    ref_utms = np.array(ref_utms)
    
    query_utms = []
    for city in val_cities:
        csv_path = os.path.join(msls_path, 'train_val', city, 'query', 'subtask_index.csv')
        raw_path = os.path.join(msls_path, 'train_val', city, 'query', 'postprocessed.csv')
        idx_df = pd.read_csv(csv_path)
        raw_df = pd.read_csv(raw_path)
        val_keys = idx_df[idx_df['all'] == True]['key'].tolist()
        for key in val_keys:
            row = raw_df[raw_df['key'] == key].iloc[0]
            query_utms.append([row['easting'], row['northing']])
    query_utms = np.array(query_utms)
...
class SPEDDataset(Dataset):
    def __init__(self, dataset_path, split='ref', transform=None):
        ...
        self.image_paths = sorted(glob.glob(
            os.path.join(dataset_path, split, '**', '*.jpg'), recursive=True
        ) + glob.glob(
            os.path.join(dataset_path, split, '**', '*.png'), recursive=True
        ))
...
class MSLSDataset(Dataset):
    def __init__(self, dataset_path, cities, split='database', transform=None):
        ...
        for city in cities:
            ...
            val_keys = df[df['all'] == True]['key'].tolist()
            for key in val_keys:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ckpt_dir', type=str, default=None,
                        help='folder containing multiple checkpoints')
    ...
    save_dir = './LOGS/eval_results'
    os.makedirs(save_dir, exist_ok=True)

    summary_file = os.path.join(save_dir, 'all_results_summary.csv')
    summary_rows = []
    ...
    for ckpt_path in ckpt_list:
        ...
        for ds in args.datasets:
            try:
                if ds == 'nordland':
                    pitts_dict = eval_nordland(model, batch_size=args.batch_size, device=device)
                elif ds == 'pitts250k':
                    pitts_dict = eval_pitts250k(model, batch_size=args.batch_size, device=device)
                elif ds == 'sped':
                    pitts_dict = eval_sped(model, batch_size=args.batch_size, device=device)
                elif ds == 'msls':
                    pitts_dict = eval_msls(model, batch_size=args.batch_size, device=device)

                row[f'{ds}_R1']  = round(pitts_dict[1], 4)
                row[f'{ds}_R5']  = round(pitts_dict[5], 4)
                row[f'{ds}_R10'] = round(pitts_dict[10], 4)

                result_file = os.path.join(save_dir, f'{ckpt_name}_{ds}.txt')
                with open(result_file, 'w') as f:
                    f.write(f"Checkpoint: {ckpt_path}\n")
                    f.write("="*50 + "\n")
                    f.write(f"\n{ds.upper()}\n")
                    f.write(f"R@1:  {pitts_dict[1]:.4f}\n")
                    f.write(f"R@5:  {pitts_dict[5]:.4f}\n")
                    f.write(f"R@10: {pitts_dict[10]:.4f}\n")
    ...
    print(f"\nAll done. Final summary: {summary_file}")

'''
python eval.py \
  --ckpt_dir /media/hdd/ihsuan/ClipVPR/livpr/LOGS/resnet50/lightning_logs/Final_ResNet_DepthAnythingV2_DINOv2/checkpoints \
